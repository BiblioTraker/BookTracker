...
// Définition des props du composant Statistics
interface StatisticsProps {
  books: Book[];
  onFilter: (filter: string) => void;
}

const Statistics: React.FC<StatisticsProps> = ({ books, onFilter }) => {
  // Fonction pour compter le nombre de livres par statut
  const getStatusCounts = (): PieChartData[] => {
  ...
  const data: PieChartData[] = getStatusCounts();
  const COLORS = ["#0088FE", "#00C49F", "#FFBB28", "#FF8042"];

  // Gérer le clic sur un segment du PieChart
  const handleClick = (data: PieChartData): void => {
    onFilter(data.name);
  };
        ...
        <div className="flex">
          ...
          <PieChart width={600} height={450}>
            <Pie
              ...
              fill="#8884d8"
              label
              onClick={handleClick} // ✅ Fonction correctement typée
            >
